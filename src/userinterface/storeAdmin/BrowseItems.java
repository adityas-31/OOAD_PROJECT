...
public class BrowseItems extends javax.swing.JPanel {
    ...
    public BrowseItems(JPanel userProcessContainer, brandDirectory brandDirectory, Store store) {
        this.store = store;
    ...
    public void populatebrandCombo() {
        brandComboBox1.removeAllItems();

        for (brand brand : brandDirectory.getStoreList()) {
            brandComboBox1.addItem(brand);
        }
        populatebrandTable();
    ...
    public void populatebrandTable() {

        DefaultTableModel dtm = (DefaultTableModel) ItemTable.getModel();
        dtm.setRowCount(0); // to make row count 0
        // int rowCount = accountJTable.getRowCount();
        brand brand = (brand) brandComboBox1.getSelectedItem();
        if (brand != null) {
            for (Item Item : brand.getItemCatalog().getItemList()) {
                Object[] row = new Object[4];
                row[0] = Item;
                row[1] = Item.getItemID();
                row[2] = Item.getItemPrice();
                row[3] = Item.getItemAvailibility();
    ...
    private void initComponents() {
        ...
        brandComboBox1 = new javax.swing.JComboBox();
        ...
        brandComboBox1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                brandComboBox1ActionPerformed(evt);
            }
        });
        add(brandComboBox1, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 140, 250, -1));

        jLabel1.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        jLabel1.setText("brand Company");
        add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 110, 170, 30));

        jLabel2.setFont(new java.awt.Font("Tahoma", 0, 18)); // NOI18N
        jLabel2.setText("brand Item Catalog");
        ...
        viewProdjButton2.setText("View Item Detail");
        ...
        btnSearchProduct.setText("Search Item");
        ...
        jLabel3.setText("Order items from CVS brand");
    ...
    private void brandComboBox1ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_brandComboBox1ActionPerformed
        // TODO add your handling code here:
        populatebrandTable();
        // to make sure products are displayed by default .

    }// GEN-LAST:event_brandComboBox1ActionPerformed
    ...
    private void btnBackActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnBackActionPerformed
        ...
        if (order.getOrderItemList().size() > 0) {
            for (OrderItem oi : order.getOrderItemList()) {
                ...
                Item.setItemAvailibility(oi.getQuantity() + Item.getItemAvailibility());
    ...
    private void viewProdjButton2ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_viewProdjButton2ActionPerformed
        ...
        Item Item = (Item) ItemTable.getValueAt(selectedRow, 0);
        ViewItemDetailJPanel viewItemDetails = new ViewItemDetailJPanel(userProcessContainer, Item);
        userProcessContainer.add("viewItemDetails", viewItemDetails);
        CardLayout layout = (CardLayout) userProcessContainer.getLayout();
        layout.next(userProcessContainer);
    ...
    private void addtoCartButton6ActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_addtoCartButton6ActionPerformed
        ...
        if (selectedRow < 0) {
            ...
            selectedItem = (Item) ItemTable.getValueAt(selectedRow, 0);
        }
        ...
        if (salesPrice < selectedItem.getItemPrice()) {
            JOptionPane.showMessageDialog(null, "sales price should be greater than supplier price", "warning",
                    JOptionPane.INFORMATION_MESSAGE);
            return;
        }
        int fetchQuantity = (Integer) quantitySpinner.getValue();// interger type cast coz getvalue sends a n object wch
                                                                 // has to be casted to int
        if (fetchQuantity <= 0) {
            JOptionPane.showMessageDialog(null, "Please select atleast 1 quantity", "warning",
                    JOptionPane.ERROR_MESSAGE);
            return;
        } else if (fetchQuantity <= selectedItem.getItemAvailibility()) {
            boolean alreadyPresent = false;
            for (OrderItem orderItems : order.getOrderItemList()) {
                if (orderItems.getItem() == selectedItem) {
                    int oldAvailibility = selectedItem.getItemAvailibility();
                    int newAvailibility = oldAvailibility - fetchQuantity;
                    selectedItem.setItemAvailibility(newAvailibility);
            ...
            if (!alreadyPresent) {
                ...
                int newAvailibility = oldAvailibility - fetchQuantity;
                selectedItem.setItemAvailibility((newAvailibility));
                order.addOrderItem(selectedItem, fetchQuantity, salesPrice);
                refreshOrderTable();
                populatebrandTable();
            }
        } else {
    ...
    private void btnModifyQuantityActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnModifyQuantityActionPerformed
        ...
        if (!txtNewQuantity.getText().isEmpty() && !txtNewQuantity.getText().equals("0")) {
            ...
            int currentAvailibility = orderItem.getItem().getItemAvailibility();
            int oldQty = orderItem.getQuantity();
            int newQty = Integer.parseInt(txtNewQuantity.getText());

            if (newQty > (currentAvailibility + oldQty)) {
                JOptionPane.showMessageDialog(null, "qty is more than the available one", "warning",
                        JOptionPane.ERROR_MESSAGE);
                return;
            }
            orderItem.setQuantity(newQty);
            orderItem.getItem().setItemAvailibility(currentAvailibility + (oldQty - newQty));
    ...
    private void btnSearchProductActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnSearchProductActionPerformed
        ...
        if (txtSearchKeyWord.getText().trim().length() >= 0) {
            try {
                int ItemID = Integer.parseInt(txtSearchKeyWord.getText());
                Item Item = brandDirectory.searchItem(ItemID);
                if (Item != null) {
                    ViewItemDetailJPanel viewItemDetail = new ViewItemDetailJPanel(userProcessContainer, Item);
                    userProcessContainer.add("viewItemDetail", viewItemDetail);
                    CardLayout layout = (CardLayout) userProcessContainer.getLayout();
                    layout.next(userProcessContainer);
                } else {
                    JOptionPane.showMessageDialog(null, "No such Item is present", "Warning",
                            JOptionPane.INFORMATION_MESSAGE);
                }
                errorInkeyword.setText("");
            } catch (NumberFormatException nfe) {
                errorInkeyword.setText("Enter a Item ID .");
            }
    ...
    private void btnRemoveOrderItemActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_btnRemoveOrderItemActionPerformed
        ...
        OrderItem orderItem = (OrderItem) orderTable.getValueAt(selectedRow, 0);
        int oldQty = orderItem.getItem().getItemAvailibility();
        int newQty = oldQty + orderItem.getQuantity();
        orderItem.getItem().setItemAvailibility(newQty);
        order.removeOrderItem(orderItem);
        populatebrandTable();
        JOptionPane.showMessageDialog(null, "order item is removed successfully", "Order item deletion",
                JOptionPane.INFORMATION_MESSAGE);
    ...
    private javax.swing.JComboBox brandComboBox1;
